...
def copy = new CopyToPDS().file(new File("/u/VASUKI/build/Helloworld.cbl")).dataset("VASUKI.BUILD.COBOL").member("HELLO")
copy.execute()

println("Compiling . . .")
def compile = new MVSExec().pgm("IGYCRCTL").parm("LIB")
compile.dd(new DDStatement().name("SYSIN").dsn("VASUKI.BUILD.COBOL(HELLO)").options("shr"))
compile.dd(new DDStatement().name("SYSLIN").dsn("VASUKI.BUILD.OBJ(HELLO)").options("shr"))
...
compile.copy(new CopyToHFS().ddName("SYSPRINT").file(new File("/u/VASUKI/build/helloworld.log")))
